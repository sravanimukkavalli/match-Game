...
class MatchComponent extends Component {
  ...
  checkinMainImgWithThumbnail = id => {
    ...
    const lengthOfImages = imagesList.length - 1
    const num = Math.ceil(Math.random() * lengthOfImages)
    const image = imagesList[num]
    console.log(image)
    const userclickedImg = imagesList.filter(each => each.id === id)
    console.log(userclickedImg, mainImageUrl)
    if (mainImageUrl.id === userclickedImg[0].id) {
      this.setState(prevState => ({
      ...
      this.setState({isMatch: false}, this.componentWillUnmount)
    }
  }
  ...
  render() {
    const {count, time, mainImageUrl, activeTabId, isMatch} = this.state
    const imagesRelatedToActiveTab = imagesList.filter(
      each => each.category === activeTabId,
    )

    return (
      <div className="bg-container">
        <ul className="header-container fixed-top">
          <li>
            <img
        ...
        {isMatch ? (
          <div className="main-container">
            <img className="main-img" src={mainImageUrl.imageUrl} alt="match" />
            ...
            <ul className="thumbnails-container">
              {imagesRelatedToActiveTab.map(eachImage => (
                <ThumbnailItem
                  key={eachImage.id}
                  imageDetails={eachImage}
                  checkinMainImgWithThumbnail={this.checkinMainImgWithThumbnail}
                />
              ))}
            </ul>
          </div>
          ...
          <div className="result-container">
            ...
            <p className="your-score-text">{count}</p>
